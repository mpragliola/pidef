...
test.describe('Navigation', () => {
  ...
  test.beforeAll(async () => {
    ...
    await expect(window.locator('#nav-label')).toHaveText('Page 1 / 3', {
      timeout: 15000,
    });
  });
  ...
  test('next button advances to page 2', async () => {
    ...
    await expect(window.locator('#nav-label')).toHaveText('Page 2 / 3');
  });
  ...
  test('last button jumps to final page', async () => {
    ...
    await expect(window.locator('#nav-label')).toHaveText('Page 3 / 3');
  });
